    ...
    >
      ...
      >
        {[
          ...
          { label: 'Especialidades', value: data.especialidades?.join(', ') || '—', emoji: '⭐' },
...
export default function OnboardingPage() {
  ...
  // ── Continuar a Paso 3 — useCallback para evitar stale closure ───────────
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      // ── FIX: done y extracted son independientes ──────────────────────
      if (extracted) {
      ...
      if (done) {
        setTimeout(() => {
          setIsComplete(true);
          // Auto-navega a step2 tras 2s si el usuario no hace clic
          setTimeout(() => navigate('/register/step2', { replace: true }), 2000);
        }, 500);
      }
  ...
  }, [inputValue, chatHistory, sendingToAPI, setGeminiData, navigate]);
